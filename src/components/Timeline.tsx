...
const steps = [
  {
    ...
    items: ["Importa tu presupuesto desde Excel", "Invita a tu equipo (roles automáticos)", "Conecta con QuickBooks o SAP", "Dashboard activo al instante"],
    accent: "#C084FC",
  },
  {
    day: "Día 5",
    ...
    items: ["Órdenes de compra desde el móvil", "Alertas de desviación configuradas", "Proveedores con acceso limitado", "Primer reporte automático enviado"],
  ...
  },
  {
    ...
    items: ["100% del gasto centralizado", "0 reportes manuales", "ROI demostrable (28% menos sobrecosto)", "Decisiones en tiempo real"],
    accent: "#911ECC",
  },
];
...
export default function Timeline() {
  ...
  return (
    <section className="section-pad-lg bg-[#0F0B20] relative overflow-hidden" ref={ref}>
      ...
      <div className="absolute top-1/2 left-0 w-[300px] h-[300px] bg-[#911ECC] opacity-[0.05] blur-[80px] rounded-full pointer-events-none" />
      ...
      <div className="container-xl relative z-10">
        ...
        >
          <p className="text-white/30 text-xs uppercase tracking-[0.15em] font-medium mb-4">Implementación</p>
          ...
          <p className="text-body-lg text-white/45">
            Mientras otros softwares tardan meses, Mawi funciona el día 1.
          </p>
        </motion.div>

        {/* Timeline */}
        <div className="relative max-w-3xl mx-auto">
          <div className="absolute left-[19px] top-0 bottom-0 w-px bg-white/5 md:left-1/2" />
          <div className="absolute left-[19px] top-0 bottom-0 w-px overflow-hidden md:left-1/2">
            <motion.div
              className="w-full origin-top"
              style={{ height: lineHeight, background: "linear-gradient(to bottom, #C084FC, #911ECC, rgba(145,30,204,0.2))" }}
            />
          </div>
            ...
            >
              ...
              <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 top-0 flex-shrink-0 z-10">
                <motion.div
                  ...
                  className="w-10 h-10 rounded-full flex items-center justify-center bg-[#0F0B20] border border-white/10"
              ...
              <div className={`pl-16 md:pl-0 ${i % 2 === 0 ? "md:pr-14 md:text-right" : "md:pl-14"} md:w-1/2`}>
                ...
                <ul className={`space-y-2 ${i % 2 === 0 ? "md:flex md:flex-col md:items-end" : ""}`}>
                  {step.items.map((item) => (
                    <li key={item} className={`flex items-center gap-2 text-white/50 text-sm ${i % 2 === 0 ? "md:flex-row-reverse" : ""}`}>
