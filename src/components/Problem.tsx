...
const pains = [
  { stat: 28, suffix: "%",   label: "Sobrecosto promedio",     desc: "Los proyectos terminan costando 28% más de lo planeado.",              color: "rgba(255,255,255,0.35)" },
  { stat: 6,  suffix: "h",   label: "Perdidas diario en admin", desc: "Tu equipo gasta el día en tareas manuales que no agregan valor.",       color: "rgba(255,255,255,0.35)" },
  { stat: 9,  suffix: "/10", label: "Sin visibilidad real",     desc: "9 de 10 constructoras no saben el estado real de sus finanzas.",        color: "rgba(255,255,255,0.35)" },
  { stat: 72, suffix: "h",   label: "Retraso promedio",         desc: "Cuando te enteras del problema, ya es demasiado tarde para actuar.",   color: "rgba(255,255,255,0.35)" },
];
...
export default function Problem() {
  ...
  return (
    <section id="problema" className="section-pad-lg bg-[#080514] relative overflow-hidden" ref={ref}>
      <div className="absolute inset-0 bg-dots opacity-25" />
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#911ECC] opacity-[0.06] blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="container-xl relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-1.5 mb-5">
            ...
            <span className="text-white/50 text-xs font-semibold uppercase tracking-wider">El problema real</span>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-12">
          {pains.map((p, i) => (
            ...
            >
              <div className="w-2 h-2 rounded-full bg-[#911ECC] mb-4" />
              <p className="text-4xl font-bold text-white mb-2 leading-none">
                ...
                <span className="text-white/50">{p.suffix}</span>
              </p>
              <h3 className="text-white/80 font-semibold mb-2 text-sm">{p.label}</h3>
        ...
        >
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-white/60 font-bold text-lg">Sin Mawi</h3>
            </div>
            <ul className="space-y-3">
              {[
                ...
                <li key={item} className="flex items-start gap-3 text-white/30 text-sm">
                  <X size={13} className="text-white/20 mt-0.5 flex-shrink-0" />
                  {item}
                </li>
              ))}
            </ul>
          ...
          {/* After */}
          <div className="rounded-2xl border border-[#911ECC]/25 bg-[#911ECC]/6 p-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded-full bg-[#911ECC]/20 border border-[#911ECC]/30 flex items-center justify-center">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={item} className="flex items-start gap-3 text-white/65 text-sm">
                  <Check size={13} className="text-[#C084FC] mt-0.5 flex-shrink-0" />
                  {item}
                </li>
              ))}
            </ul>
